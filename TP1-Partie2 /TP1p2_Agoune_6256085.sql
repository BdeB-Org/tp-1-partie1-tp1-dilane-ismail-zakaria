...
SELECT 
    prenom ||' '|| nom_famille AS "Nom Complet"
FROM 
    outils_usager;
...
FROM 
    outils_emprunt
WHERE 
    date_emprunt < TO_DATE('2014-01-01', 'YYYY-MM-DD');
...
FROM 
    outils_outil
WHERE 
    caracteristiques NOT LIKE '%20 volt%';
...
SELECT 
    ...
    COALESCE(e.date_retour,TO_DATE(SYSDATE,'YY-MM-DD'))-e.date_emprunt AS "Durée de l'emprunt (jours)",
...
FROM 
    outils_outil
WHERE 
    fabricant ='Makita' AND prix > 
        (SELECT AVG(prix) from outils_outil);
